...
class PlaylistListBloc with StreamSubscriberMixin {
  PlaylistListBloc({
  ...
  String _channelId;
  String _apiKey;
  final log = Logger('PlaylistListBloc');
...
//only make it false if first page is fetched successfully
    ...
    try {
      Playlist playlist =
          await _youtubeApi.getPlaylistList(_channelId, _apiKey, _nextPage);
      List<PlaylistItem> list = playlist.itemList;
      ...
      _listItems
          .addAll(list.map((playlistItem) => PlaylistListItem(playlistItem)));
      _nextPage = playlist.nextPageToken;
      _dataSubject.add(_listItems);
    ...
    if (!_isNextLoading) {
      ...
      if (_nextPage != null && _nextPage.isNotEmpty) {
        ...
        try {
          ...
          _listItems.addAll(
              list.map((playlistItem) => PlaylistListItem(playlistItem)));
